...
import 'package:tamam_business/features/advertisement/models/popup_menu_model.dart';
import 'package:tamam_business/features/advertisement/widgets/ads_create_success_bottom_sheet.dart';
...
import 'package:tamam_business/helper/image_size_checker.dart';
...
class AdvertisementController extends GetxController implements GetxService {
  ...
  bool validateTimeRange(TextEditingController? timeRange) {
    ...
    if (timeRange != null) {
      ...
      try {
        networkStartDate =
            LocaleHelper.parseDate(formattedString, "MM/dd/yyyy")?.toLocal() ?? DateTime.now();
      } catch (e) {
        try {
          networkStartDate =
              LocaleHelper.parseDate(formattedString, 'd MMM,y')?.toLocal() ?? DateTime.now();
        } catch (e2) {
          networkStartDate = DateTime.now();
        }
      }
  ...
  Future<void> submitNewAdvertisement(
    ...
    Map<String, String> body = {
      ...
      'dates':
          "${DateConverter.dateTimeForCoupon(dateTimeRange?.start ?? DateTime.now())} - ${DateConverter.dateTimeForCoupon(dateTimeRange?.end ?? DateTime.now())}",
