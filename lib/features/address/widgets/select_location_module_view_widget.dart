...
import 'package:tamam_business/features/address/widgets/location_search_dialog_widget.dart';
import 'package:tamam_business/features/address/widgets/module_view_widget.dart';
import 'package:tamam_business/features/address/widgets/permission_dialog_widget.dart';
import 'package:tamam_business/features/address/widgets/pickup_zone_widget.dart';
import 'package:tamam_business/features/address/widgets/zone_selection_widget.dart';
...
class _SelectLocationAndModuleViewWidgetState extends State<SelectLocationAndModuleViewWidget> {
  ...
  Widget mapView(AddressController addressController) {
    return addressController.zoneList!.isNotEmpty ? Center(
      child: Container(
        height: widget.fromView ? 250 : (context.height * 0.87),
              ...
              onCameraIdle: () {
                try {
                  if (_cameraPosition != null &&
                      ...
                      addressController.selectedZoneIndex != null &&
                      addressController.selectedZoneIndex! >= 0 &&
                      addressController.selectedZoneIndex! < addressController.zoneList!.length) {
                  ...
                  if(!widget.fromView && widget.mapController != null && _cameraPosition != null) {
                    widget.mapController!.moveCamera(CameraUpdate.newCameraPosition(_cameraPosition!));
                  }
                } catch (e) {
                  debugPrint('Camera idle error: $e');
                }
              },
              ...
              child: InkWell(
                onTap: () async {
                  ...
                  if(position != null) {
                    ...
                    if(!widget.fromView && _cameraPosition != null) {
                      widget.mapController!.moveCamera(CameraUpdate.newCameraPosition(_cameraPosition!));
                      addressController.setLocation(
                        _cameraPosition!.target, forStoreRegistration: true,
                        zoneId: addressController.zoneList![addressController.selectedZoneIndex!].id,
  ...
  _setPolygon(ZoneModel zoneModel) {
    ...
    polygonList.add(
      Polygon(
        ...
        fillColor: Get.theme.colorScheme.primary.withAlpha((.2 * 255).round()),
      ),
    );
