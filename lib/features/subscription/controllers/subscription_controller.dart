...
import 'package:tamam_business/features/subscription/domain/models/subscription_transaction_model.dart';
import 'package:tamam_business/features/subscription/domain/services/subscription_service_interface.dart';
import 'package:tamam_business/features/subscription/widgets/renew_subscription_plan_bottom_sheet.dart';
import 'package:tamam_business/features/subscription/widgets/subscription_dialog_widget.dart';
import 'package:tamam_business/features/subscription/widgets/trial_end_bottomsheet.dart';
...
class SubscriptionController extends GetxController implements GetxService {
  ...
  Future<ResponseModel?> renewBusinessPlan(
      {required String storeId, required bool isCommission}) async {
    _isLoading = true;
    update();
  ...
  Future<void> cancelSubscription(int storeId, int subscriptionId) async {
    ...
    Response response = await subscriptionServiceInterface.cancelSubscription(
        {'store_id': '$storeId', 'subscription_id': '$subscriptionId'});
    if (response.statusCode == 200) {
      if (Get.find<AuthController>().getModuleType() == 'rental') {
        await Get.find<TaxiProfileController>().getProfile();
      }
  ...
  Future<void> getSubscriptionTransactionList(
    ...
    if (!_offsetList.contains(offset)) {
      ...
      SubscriptionTransactionModel? subscriptionTransactionModel =
        ...
        from: from,
        to: to,
        restaurantId: Get.find<ProfileController>().profileModel!.stores![0].id,
  ...
  Future<bool> trialEndBottomSheet() async {
    // 🚨 APPLE COMPLIANCE: Only show trial end dialog if subscription management is enabled
    if (!RemoteConfigHelper.isSubscriptionManagementEnabled()) {
      return true; // Skip trial end dialog when subscription management is disabled
    }
