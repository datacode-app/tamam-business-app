...
import 'package:tamam_business/features/store/domain/models/attribute_model.dart';
import 'package:tamam_business/features/store/domain/models/band_model.dart';
import 'package:tamam_business/features/store/domain/models/item_model.dart';
import 'package:tamam_business/features/store/domain/models/pending_item_model.dart';
import 'package:tamam_business/features/store/domain/models/review_model.dart';
import 'package:tamam_business/features/store/domain/models/suitable_tag_model.dart';
import 'package:tamam_business/features/store/domain/models/unit_model.dart';
import 'package:tamam_business/features/store/domain/models/variant_type_model.dart';
import 'package:tamam_business/features/store/domain/models/variation_body_model.dart';
...
class StoreController extends GetxController implements GetxService {
  ...
  String _fleetAssignmentMode = 'auto';
  String get fleetAssignmentMode => _fleetAssignmentMode;

  String _preferredDriverSelection = 'nearest';
  String get preferredDriverSelection => _preferredDriverSelection;

  final List<String> _durationType = ['minutes', 'hours', 'days'];
  List<String> get durationType => _durationType;
  ...
  Future<void> updateStore(Store store, String min, String max, String type,
    ...
    _isLoading = true;
    update();
    store.selfDeliverySystem = _isSelfDeliveryEnabled ? 1 : 0;
    store.fleetAssignmentMode = _fleetAssignmentMode;
    store.preferredDriverSelection = _preferredDriverSelection;
  ...
  void initStoreData(Store store) {
    _rawLogo = null;
    _rawCover = null;
    _isGstEnabled = store.gstStatus ?? false;
    _isStoreVeg = store.veg == 1;
    _isStoreNonVeg = store.nonVeg == 1;
    _isScheduleOrderEnabled = store.scheduleOrder ?? false;
    _isDeliveryEnabled = store.delivery ?? false;
    _isTakeAwayEnabled = store.takeAway ?? false;
    _isCutleryEnabled = store.cutlery ?? false;
    _isFreeDeliveryEnabled = store.freeDelivery ?? false;
    _isExtraPackagingEnabled = store.extraPackagingStatus ?? false;
    _isPrescriptionStatusEnable = store.prescriptionStatus ?? false;
    _isHalal = store.isHalalActive ?? false;
    _isSelfDeliveryEnabled = store.selfDeliverySystem == 1;
    _fleetAssignmentMode = store.fleetAssignmentMode ?? 'auto';
    _preferredDriverSelection = store.preferredDriverSelection ?? 'nearest';
  ...
  void setVeg(bool isVeg, bool? isStoreVeg, bool? isStoreNonVeg) {
    _isVeg = isVeg;
    _isStoreVeg = isStoreVeg;
    _isStoreNonVeg = isStoreNonVeg;
    update();
  }
  ...
  void toggleSelfDelivery() {
    _isSelfDeliveryEnabled = !_isSelfDeliveryEnabled;
    update();
  }
  ...
  void setFleetAssignmentMode(String mode) {
    _fleetAssignmentMode = mode;
    update();
  }

  void setPreferredDriverSelection(String selection) {
    _preferredDriverSelection = selection;
    update();
  }
}
