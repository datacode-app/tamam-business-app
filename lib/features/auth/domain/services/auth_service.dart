  ...
  Future<ResponseModel?> manageLogin(Response response, String type) async {
    ...
    if (response.statusCode == 200) {
      ...
      if(response.body['subscribed'] != null){
        ...
        // 🚨 APPLE COMPLIANCE: Only show subscription flows if remote config allows
        if(packageId == null) {
          ...
          // Only navigate to subscription route if subscription management is enabled
          ...
          // Only show subscription payment screen if subscription management is enabled
          if (RemoteConfigHelper.isSubscriptionManagementEnabled()) {
            ...
            // Skip subscription payment when disabled
            saveUserToken(response.body['subscribed']['token'], response.body['subscribed']['zone_wise_topic'], type);
            await updateToken();
            moduleType == 'rental' ? await Get.find<TaxiProfileController>().getProfile() : await Get.find<ProfileController>().getProfile();
          }
