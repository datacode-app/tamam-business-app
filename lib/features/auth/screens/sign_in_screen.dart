...
import 'package:tamam_business/common/widgets/developer_settings_dialog.dart';
import 'package:tamam_business/features/auth/controllers/auth_controller.dart';
import 'package:tamam_business/features/auth/widgets/store_registartion_success_bottom_sheet.dart';
          ...
          child: Center(
            child: SizedBox(
              ...
              child: GetBuilder<AuthController>(builder: (authController) {
                ...
                debugPrint('====> Sign-in screen: config model loaded: ${configModel != null}');
                debugPrint('====> Sign-in screen: toggle store registration: ${configModel?.toggleStoreRegistration}');
                
                // Check vendor registration permission using remote config + server fallback
                final isVendorRegistrationEnabled = RemoteConfigHelper.isVendorRegistrationEnabled() && 
                    (configModel?.toggleStoreRegistration ?? true);
                
                debugPrint('====> Final vendor registration decision: $isVendorRegistrationEnabled');
                debugPrint('====> Remote config: ${RemoteConfigHelper.isVendorRegistrationEnabled()}');
                debugPrint('====> Server config: ${configModel?.toggleStoreRegistration}');

                return Column(children: [
                  GestureDetector(
                    onTap: () async {
                      final activated = await DeveloperConfigHelper.handleLogoTap();
                      if (activated) {
                        HapticFeedback.mediumImpact();
                        Get.dialog(const DeveloperSettingsDialog());
                      }
                    },
                  ...
                  SizedBox(
                      height: isVendorRegistrationEnabled
                  ...
                  isVendorRegistrationEnabled
                      ? TextButton(
                          ...
                          onPressed: () async {
                            debugPrint('====> Navigating to vendor registration (remote config enabled)');
                            Get.toNamed(
