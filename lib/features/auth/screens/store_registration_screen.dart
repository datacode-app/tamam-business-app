...
import 'package:tamam_business/common/widgets/custom_text_field_widget.dart';
import 'package:tamam_business/common/widgets/custom_tool_tip_widget.dart';
import 'package:tamam_business/features/address/controllers/address_controller.dart';
import 'package:tamam_business/features/address/widgets/select_location_module_view_widget.dart';
import 'package:tamam_business/features/auth/controllers/auth_controller.dart';
import 'package:tamam_business/features/auth/widgets/custom_time_picker_widget.dart';
import 'package:tamam_business/features/auth/widgets/pass_view_widget.dart';
import 'package:tamam_business/features/business/domain/models/package_model.dart';
import 'package:tamam_business/features/business/widgets/base_card_widget.dart';
import 'package:tamam_business/features/business/widgets/package_card_widget.dart';
...
import 'package:tamam_business/features/store/domain/models/store_body_model.dart';
import 'package:tamam_business/helper/remote_config_helper.dart';
import 'package:tamam_business/helper/route_helper.dart';
import 'package:tamam_business/helper/validate_check.dart';
...
class _StoreRegistrationScreenState extends State<StoreRegistrationScreen> with TickerProviderStateMixin {
  ...
  int get _safeTabIndex {
    if (_tabController == null) return 0;
    final index = _tabController!.index;
    return index.clamp(0, _nameController.length - 1);
  }
  ...
  void initState() {
    ...
    // Ensure we have at least one language for proper initialization
    if (_languageList.isEmpty) {
      _languageList.add(Language(key: 'en', value: 'English'));
    }
    
    final int languageLength = _languageList.length.clamp(1, 10); // Ensure at least 1 tab
    _tabController = TabController(length: languageLength, initialIndex: 0, vsync: this);
    String countryCode = Get.find<SplashController>().configModel?.country ?? 'US';
    ...
    // Defer all controller calls to avoid setState during build
    WidgetsBinding.instance.addPostFrameCallback((_) {
      try {
        Get.find<AuthController>().storeStatusChange(0.1, isUpdate: false);
        Get.find<AddressController>().getZoneList();
        final authController = Get.find<AuthController>();
        if (authController.deliveryTimeTypeList.isNotEmpty) {
          authController.setDeliveryTimeTypeIndex(authController.deliveryTimeTypeList[0], false);
        }
        if(authController.showPassView){
          authController.showHidePass(isUpdate: false);
        }
        authController.pickImageForReg(false, true);
        authController.resetBusiness();
        Get.find<AddressController>().clearPickupZone();
      } catch (e) {
        debugPrint('====> Error initializing store registration controllers: $e');
      }
    });
  ...
  Widget build(BuildContext context) {
    // Check if vendor registration is allowed via remote config
    final configModel = Get.find<SplashController>().configModel;
    final isVendorRegistrationEnabled = RemoteConfigHelper.isVendorRegistrationEnabled() && 
        (configModel?.toggleStoreRegistration ?? true);
    
    // If vendor registration is disabled, show a blocked screen
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  'vendor_registration_temporarily_unavailable'.tr,
                ...
                Text(
                  'please_contact_support_for_vendor_registration'.tr,
                  style: robotoRegular.copyWith(
                    color: Theme.of(context).textTheme.bodyLarge?.color?.withAlpha((0.7 * 255).round()),
                  ),
                  textAlign: TextAlign.center,
                ),
    ...
    return GetBuilder<AuthController>(builder: (authController) {
      return GetBuilder<AddressController>(builder: (addressController) {
        ...
        // Safety check: Ensure all required data is initialized
        if (_tabController == null || _nameController.isEmpty || _addressController.isEmpty) {
        ...
        if(addressController.storeAddress != null && _languageList.isNotEmpty && _addressController.isNotEmpty){
                        ...
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          ...
                          Container(
                            ...
                            child: Column(children: [
                              ...
                              CustomTextFieldWidget(
                                ...
                                controller: _nameController[_safeTabIndex],
                                focusNode: _nameFocus[_safeTabIndex],
                                nextFocus: _safeTabIndex != (_languageList.length-1) ? _addressFocus[_safeTabIndex] : _addressFocus[0],
                              ...
                              Row(children: [
                                ...
                                Expanded(flex: 4,
                                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                    ...
                                    Row(children: [
                                      Text('vendor_logo'.tr, style: robotoRegular.copyWith(color: Theme.of(context).textTheme.bodyLarge?.color?.withAlpha((0.7 * 255).round()))),
                                        ...
                                        child: ClipRRect(
                                          ...
                                          child: authController.pickedLogo != null ? GetPlatform.isWeb ? Image.network(
                                            ...
                                            child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                                              ...
                                              Icon(CupertinoIcons.photo_camera_solid, size: 30, color: Theme.of(context).disabledColor.withAlpha((0.6 * 255).round())),
                                              const SizedBox(height: Dimensions.paddingSizeSmall),
                                              ...
                                              Padding(
                                                ...
                                                child: Text(
                                                  ...
                                                  style: robotoRegular.copyWith(color: Theme.of(context).textTheme.bodyLarge?.color?.withAlpha((0.7 * 255).round())), textAlign: TextAlign.center,
                                                ),
                                              ),
                                      ...
                                      Positioned(
                                        ...
                                        child: InkWell(
                                          onTap: authController.isLoading ? null : () {
                                            debugPrint('Logo image tap detected');
                                ...
                                Expanded(flex: 6,
                                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                    ...
                                    Row(children: [
                                      Text('vendor_cover'.tr, style: robotoRegular.copyWith(color: Theme.of(context).textTheme.bodyLarge?.color?.withAlpha((0.7 * 255).round()))),
                                    ...
                                    Stack(children: [
                                            ...
                                            child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                                              ...
                                              Padding(
                                                ...
                                                child: Text(
                                                  ...
                                                  style: robotoRegular.copyWith(color: Theme.of(context).disabledColor.withAlpha((0.6 * 255).round()), fontSize: Dimensions.fontSizeSmall),
                                                  textAlign: TextAlign.center,
                                                ),
                                              ),
                                      ...
                                      Positioned(
                                        ...
                                        child: InkWell(
                                          onTap: authController.isLoading ? null : () {
                                            debugPrint('Cover image tap detected');
                          ...
                          Container(
                            decoration: BoxDecoration(
                              color: Theme.of(context).cardColor,
                              borderRadius: BorderRadius.circular(Dimensions.radiusDefault),
                              boxShadow: [BoxShadow(color: Colors.grey.withAlpha((0.1 * 255).round()), spreadRadius: 1, blurRadius: 10, offset: const Offset(0, 1))],
                            ),
                            padding: const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeSmall, vertical: Dimensions.paddingSizeDefault),
                            child: Column(children: [
                        ...
                        child: Column(mainAxisAlignment: MainAxisAlignment.start, children: [
                          ...
                          Row(children: [
                            ...
                            CustomToolTip(
                              ...
                              preferredDirection: AxisDirection.down,
                              iconColor: Theme.of(context).textTheme.bodyLarge!.color?.withAlpha((0.7 * 255).round()),
                            ),
                          ]),
                          const SizedBox(height: Dimensions.paddingSizeDefault),
                          ...
                          Container(
                            ...
                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              ...
                              CustomTextFieldWidget(
                                ...
                                countryDialCode: _countryDialCode != null ? CountryCode.fromCountryCode(Get.find<SplashController>().configModel?.country ?? 'US').code
                      ...
                      child: Column(children: [
                        ...
                        Padding(
                          ...
                          child: Row(children: [
                            ...
                            (Get.find<SplashController>().configModel?.commissionBusinessModel ?? 0) != 0 ? Expanded(
                            ...
                            (Get.find<SplashController>().configModel?.subscriptionBusinessModel ?? 0) != 0 ? Expanded(
                        ...
                        authController.businessIndex == 0 ? Padding(
                          padding: const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeLarge),
                          child: Text(
                            "${'vendor_will_pay'.tr} ${Get.find<SplashController>().configModel?.adminCommission ?? 0}% ${'commission_to'.tr} ${Get.find<SplashController>().configModel?.businessName ?? 'Platform'} ${'from_each_order_You_will_get_access_of_all'.tr}",
                            style: robotoRegular.copyWith(fontSize: Dimensions.fontSizeSmall, color: Theme.of(context).textTheme.bodyLarge?.color?.withAlpha((0.7 * 255).round())), textAlign: TextAlign.justify, textScaler: const TextScaler.linear(1.1),
                          ),
                        ) : Column(children: [
                          Padding(
                            padding: const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeLarge),
                          ...
                          SizedBox(
                            height: 440,
                            child: authController.packageModel != null ? (authController.packageModel!.packages != null && authController.packageModel!.packages!.isNotEmpty ? Swiper(
                  ...
                  onPressed: () {
                    ...
                    if(authController.storeStatus == 0.1 || authController.storeStatus == 0.6){
                      ...
                      data.addAll(StoreBodyModel(
                        ...
                        maxDeliveryTime: maxTime, lat: addressController.restaurantLocation?.latitude.toString() ?? '0', email: email,
                        lng: addressController.restaurantLocation?.longitude.toString() ?? '0', fName: fName, lName: lName, phone: phoneWithCountryCode,
                        password: password, zoneId: (addressController.zoneList != null && addressController.selectedZoneIndex != null && addressController.zoneList!.isNotEmpty) ? addressController.zoneList![addressController.selectedZoneIndex!].id.toString() : '',
                        moduleId: (addressController.moduleList != null && addressController.selectedModuleIndex != null && addressController.moduleList!.isNotEmpty) ? addressController.moduleList![addressController.selectedModuleIndex!].id.toString() : '',
