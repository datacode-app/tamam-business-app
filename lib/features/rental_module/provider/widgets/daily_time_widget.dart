...
class DailyTimeWidget extends StatelessWidget {
                                                ...
                                                Expanded(
                                                  ...
                                                  onPressed: () {
                                                    ...
                                                    if (openingTime == null) {
                                                      ...
                                                      showCustomSnackBar(
                                                          'pick_end_time'.tr);
                                                    } else if ((LocaleHelper.parseDate(openingTime!, 'HH:mm') ?? DateTime.now())
                                                        .isAfter(LocaleHelper.parseDate(closingTime!, 'HH:mm') ?? DateTime.now())) {
                                                      showCustomSnackBar(
                    ...
                    return Padding(
                      ...
                      child: Stack(clipBehavior: Clip.none, children: [
                        Container(
                          ...
                          child: Text(
                            '${DateConverter.stringToReadableTime(scheduleList[index].openingTime!.substring(0, 5))} '
                            '- ${DateConverter.stringToReadableTime(scheduleList[index].closingTime!.substring(0, 5))}',
                          ),
                        ),
                        Positioned(
                          right: -10,
  ...
  bool _isUnderTime(String time, String startTime, String? endTime) {
    DateTime? timeDate = LocaleHelper.parseDate(time, 'HH:mm');
    DateTime? startDate = LocaleHelper.parseDate(startTime, 'HH:mm');
    DateTime? endDate = endTime != null ? LocaleHelper.parseDate(endTime, 'HH:mm') : null;
    
    if (timeDate == null || startDate == null || endDate == null) {
      return false;
    }
