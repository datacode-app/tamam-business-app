...
import 'package:tamam_business/features/rental_module/common/models/taxi_constant_type_model.dart';
...
import 'package:tamam_business/features/rental_module/provider/domain/models/taxi_review_model.dart';
import 'package:tamam_business/features/rental_module/provider/domain/models/vehicle_brand_model.dart';
import 'package:tamam_business/features/rental_module/provider/domain/models/vehicle_category_model.dart';
import 'package:tamam_business/features/rental_module/provider/domain/models/vehicle_list_model.dart';
...
class ProviderController extends GetxController implements GetxService {
  ...
  List<TextEditingController> _vinNumberControllerList = [
    TextEditingController()
  ];
  List<TextEditingController> get vinNumberControllerList =>
      _vinNumberControllerList;

  List<TextEditingController> _licensePlateNumberControllerList = [
    TextEditingController()
  ];
  List<TextEditingController> get licensePlateNumberControllerList =>
  ...
  final List<TaxiConstantTypeModel> _vehicleTypeList =
      TaxiConstantTypeModel.getVehicleTypeList();
  ...
  final List<TaxiConstantTypeModel> _fuelTypeList =
      TaxiConstantTypeModel.getFuelTypeList();
  ...
  final List<TaxiConstantTypeModel> _transmissionTypeList =
      TaxiConstantTypeModel.getTransmissionTypeList();
  ...
  final List<TaxiConstantTypeModel> _seatList =
      TaxiConstantTypeModel.getVehicleSeatList();
  List<TaxiConstantTypeModel> get seatList => _seatList;

  final List<TaxiConstantTypeModel> _coolingList =
      TaxiConstantTypeModel.getVehicleCoolingList();
  ...
  final List<TaxiConstantTypeModel> _durations =
      TaxiConstantTypeModel.getTimeDurationList();
  ...
  void initVehicleData({required Vehicles? vehicle}) {
    ...
    _discountTypeKey = vehicle?.discountType == 'percent'
        ? '%'
    ...
    _selectedVehicleType = _vehicleTypeList
        .firstWhere((element) => element.key == vehicle?.type)
        .value
        .tr;
    _selectedVehicleTypeKey = vehicle?.type;
    _selectedFuelType = _fuelTypeList
        .firstWhere((element) => element.key == vehicle?.fuelType)
        .value
        .tr;
    _selectedFuelTypeKey = vehicle?.fuelType;
    _selectedTransmissionType = _transmissionTypeList
        .firstWhere((element) => element.key == vehicle?.transmissionType)
        .value
        .tr;
    ...
    _vinNumberControllerList = [];
    _licensePlateNumberControllerList = [];
    if (vehicle?.vehicleIdentities != null &&
        vehicle!.vehicleIdentities!.isNotEmpty) {
      for (int i = 0; i < vehicle.vehicleIdentities!.length; i++) {
        _vinNumberControllerList.add(TextEditingController(
            text: vehicle.vehicleIdentities![i].vinNumber));
        _licensePlateNumberControllerList.add(TextEditingController(
            text: vehicle.vehicleIdentities![i].licensePlateNumber));
      }
    }
    update();
  }
  ...
  void pickImage(
    ...
    if (isRemove) {
      ...
      try {
        if (isLogo) {
          _logo = await providerServiceInterface.pickImageFromGallery();
        } else if (isVehicleThumbnail) {
          _vehicleThumbnail =
              await providerServiceInterface.pickImageFromGallery();
        } else {
  ...
  void pickVehicleImages() async {
    try {
      ...
      showCustomSnackBar('Error selecting image. Please try again.');
      debugPrint('Vehicle image picker error: $e');
    }
  }
  ...
  Future<XFile?> urlToXFile(String imageUrl) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        showCustomSnackBar(
            '${'failed_to_download_file'.tr} ${response.statusCode}');
        return null;
      }
    } catch (e) {
  ...
  Future<FilePickerResult?> urlToFilePickerResult(String imageUrl) async {
    try {
      ...
      showCustomSnackBar(
          'Error occurred while converting URL to FilePickerResult: $e');
      return null;
    }
  }
  ...
  Future<void> getVehicleDetails({required int vehicleId}) async {
    Vehicles? vehicleDetailsModel =
        await providerServiceInterface.getVehicleDetails(vehicleId: vehicleId);
  ...
  Future<Vehicles?> getVehicleDetailsWithTrans({required int vehicleId}) async {
    ...
    Vehicles? vehicleDetailsWithTrans =
        await providerServiceInterface.getVehicleDetailsWithTrans(vehicleId);
  ...
  Future<void> updateVehicleActivity({required int vehicleId}) async {
    bool status = await providerServiceInterface.updateVehicleActivity(
        vehicleId: vehicleId);
    if (status) {
      await getVehicleDetails(vehicleId: vehicleId);
      getVehicleList(offset: '1', search: '');
      _vehicleActive = !_vehicleActive;
      showCustomSnackBar('vehicle_activity_updated_successfully'.tr,
          isError: false);
    }
    update();
  }
  ...
  Future<void> updateVehicleNewTag({required int vehicleId}) async {
    bool status = await providerServiceInterface.updateVehicleNewTag(
        vehicleId: vehicleId);
    if (status) {
      await getVehicleDetails(vehicleId: vehicleId);
      getVehicleList(offset: '1', search: '');
      _vehicleNewTag = !_vehicleNewTag;
      showCustomSnackBar('vehicle_new_tag_updated_successfully'.tr,
          isError: false);
    }
    update();
  }
  ...
  Future<void> getVehicleList(
      {required String offset, String? search, bool willUpdate = true}) async {
    if (offset == '1') {
      _offsetList.clear();
      _offset = 1;
      _vehicleList = null;
    ...
    if (!_offsetList.contains(offset)) {
      ...
      VehicleListModel? vehicleListModel =
          await providerServiceInterface.getVehicleList(
              offset: offset,
              categoryId: _selectedCategoryId,
              brandId: _filterSelectedBrand,
              vehicleType: _filterSelectedVehicleType,
              seatingCapacity:
                  _filterSelectedSeat != null && _filterSelectedSeat!.isNotEmpty
                      ? jsonEncode(_filterSelectedSeat)
                      : _filterSelectedSeat,
              airCondition: _isCooling,
              transmissionType: _filterSelectedTransmissionType,
              fuelType: _filterSelectedFuelType,
              search: search);
  ...
  Future<void> deleteVehicle(
      {required int vehicleId, bool fromDetails = false}) async {
    _isLoading = true;
    update();

    bool status =
        await providerServiceInterface.deleteVehicle(vehicleId: vehicleId);
    if (status) {
      if (fromDetails) {
        Get.back();
      }
  ...
  Future<void> getCategoryList(
      {required String offset, bool willUpdate = true}) async {
    if (offset == '1') {
      _offsetList.clear();
      _offset = 1;
      _categories = null;
    ...
    if (!_offsetList.contains(offset)) {
      ...
      VehicleCategoryModel? vehicleCategoryModel =
          await providerServiceInterface.getCategoryList(offset: offset);
  ...
  Future<void> getBrandList(
    ...
    if (!_offsetList.contains(offset)) {
      ...
      VehicleBrandModel? vehicleBrandModel =
          await providerServiceInterface.getBrandList(offset: offset);
  ...
  Future<void> addVehicle(
      {required Vehicles? vehicle, required bool isAdd}) async {
    _isLoading = true;
    update();
    ...
    Response response = await providerServiceInterface.addVehicle(
        vehicle: vehicle,
        isAdd: isAdd,
        vehicleThumbnail: _vehicleThumbnail,
        vehicleImages: _vehicleImages,
        licenseFiles: document);
    if (response.statusCode == 200) {
      Get.back();
      setCategoryInit();
      await getVehicleList(offset: '1', search: '');
  ...
  void setSelectedDurationInitData(String duration) {
    ...
    _selectedDuration =
        _durations.firstWhere((element) => element.key == duration).value;
  }
  ...
  Future<void> updateProviderBusinessSetup(
      {required Store provider,
      required String minTime,
      required String maxTime,
      required String timeType}) async {
    _isLoading = true;
    update();
    bool status = await providerServiceInterface.updateProviderBusinessSetup(
        provider: provider,
        minTime: minTime,
        maxTime: maxTime,
        timeType: timeType,
    ...
    if (status) {
      Get.back();
      showCustomSnackBar('business_setup_updated_successfully'.tr,
          isError: false);
  ...
  Future<void> getProviderReviewList(String? searchText,
    ...
    List<TaxiReviewModel>? providerReviewList =
        await providerServiceInterface.getProviderReviewList(searchText);
  ...
  Future<void> updateReply(int reviewID, String reply,
      {bool isSent = false}) async {
    _isLoading = true;
    update();
    bool isSuccess =
        await providerServiceInterface.updateReply(reviewID, reply);
    if (isSuccess) {
      Get.back();
      showCustomSnackBar(
          isSent
              ? 'reply_sent_successfully'.tr
              : 'reply_updated_successfully'.tr,
          isError: false);
  ...
  void selectCooling(String? cooling) {
    ...
    cooling == 'air_conditioned'.tr ? _isCooling = '1' : _isCooling = '0';
    update();
  }
  ...
  bool isFilterValid() {
    return (_filterSelectedBrand != null && _filterSelectedBrand!.isNotEmpty) ||
        (_filterSelectedVehicleType != null &&
            _filterSelectedVehicleType!.isNotEmpty) ||
        (_filterSelectedSeat != null && _filterSelectedSeat!.isNotEmpty) ||
        (_isCooling != null && _isCooling!.isNotEmpty) ||
        (_filterSelectedTransmissionType != null &&
            _filterSelectedTransmissionType!.isNotEmpty) ||
        (_filterSelectedFuelType != null &&
            _filterSelectedFuelType!.isNotEmpty);
  }
